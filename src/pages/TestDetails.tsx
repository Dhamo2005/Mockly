...
import { 
  ...
  Trash2, Sliders, Download, AlarmClock, 
  RotateCcw, Calendar, AlertCircle, ArrowLeftRight, Share2, Globe, Lock, User, 
  Cloud, CloudOff, RefreshCw, Edit3
} from 'lucide-react';
...
export default function TestDetails() {
  ...
  const { tests, attempts, updateTest, activeTestSessions, clearActiveTestSession, language } = useStore();
  ...
  const { deleteTest: deleteDriveTest, deleteAttempt: deleteDriveAttempt, saveTest: saveDriveTest, isConnected, autoSync, isSyncing, lastSyncTime } = useGoogleDrive();
  ...
  const sectionsList = useMemo(() => {
    ...
    return Array.from(sectionsMap.entries()).map(([name, count]) => ({
      name,
      count,
      timeLimit: Math.floor((count / (test?.questions?.length || 1)) * (displayTimeLimit || 3600))
    }));
  }, [test]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto font-sans">
      <div className="px-3 py-4 md:py-6 md:px-6">
        <button 
          onClick={() => navigate('/tests')}
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          {/* Left Column (Hero, Sections & Questions View) */}
          <div className="w-full lg:w-2/3 xl:w-[68%] flex flex-col gap-6">
            
            {/* Hero Card */}
            <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col gap-5">
              {/* Header & Actions */}
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div className="flex-1 min-w-0 w-full space-y-2.5">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border transition-all cursor-pointer ${
                        isPublic
                    ...
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-wider rounded border border-slate-200">Medium</span>
                    
                    {test.examCategory && (
                      <span className="px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-wider rounded border border-blue-200">{test.examCategory}</span>
                    )}
                    {test.settings?.strictSectionalTiming && !test.settings?.allowSectionSwitching && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-orange-50 text-orange-700 border border-orange-200 text-[10px] font-bold uppercase tracking-wider rounded" title="Strict Sectional Timing Enabled">
                        <AlarmClock className="w-2.5 h-2.5 text-orange-500" />
                        Strict
                      </span>
                    )}
                    {test.settings?.allowSectionSwitching && (
                      <span className="flex items-center gap-1 px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-bold uppercase tracking-wider rounded" title="Section Switching Allowed">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-slate-500 leading-relaxed max-w-2xl">
                      {test.description || "Comprehensive mock examination paper"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
                  <button
                    onClick={() => setShowShareModal(true)}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 bg-white hover:bg-blue-50 rounded-lg transition-colors border border-slate-200 shadow-xs cursor-pointer"
                  >
                  ...
                  <div
                    className="p-1.5 flex items-center justify-center rounded-lg cursor-help text-slate-400"
                    title={isConnected ? (autoSync ? (isSyncing ? "Syncing..." : `Synced to Drive (${new Date(lastSyncTime || Date.now()).toLocaleTimeString()})`) : "Drive Sync Paused") : "Not Connected to Drive"}
                  >
                    {isSyncing ? <RefreshCw className="w-4 h-4 animate-spin text-blue-500" /> :
                     isConnected && autoSync ? <Cloud className="w-4 h-4 text-emerald-500" /> :
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="flex flex-col">
                  <span className="text-2xl font-bold text-slate-800 leading-none">{test.questions?.length || 0}</span>
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mt-1.5">Questions</span>
                </div>
                <div className="flex flex-col sm:border-l border-slate-100 sm:pl-5">
                  <span className="text-2xl font-bold text-slate-800 leading-none">{Math.floor(displayTimeLimit / 60)} <span className="text-base text-slate-500 font-medium">m</span></span>
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mt-1.5">Duration</span>
                </div>
                <div className="flex flex-col sm:border-l border-slate-100 sm:pl-5">
                  <span className="text-2xl font-bold text-slate-800 leading-none">{sectionsList.length}</span>
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mt-1.5">Sections</span>
                </div>
                <div className="flex flex-col sm:border-l border-slate-100 sm:pl-5">
                  <span className="text-sm font-bold text-slate-800 leading-tight flex items-center gap-1.5 mt-1.5">
                    <Calendar className="w-4 h-4 text-blue-600" />
                    {getTestDisplayDate(test)}
                  </span>
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mt-1.5">Paper Date</span>
                </div>
              </div>
            </div>

            {/* Removed Section-by-Section Questions View as per user request to only show questions on test or editing sections */}


          </div>

          {/* Right Column (Test Scheme, Attempt, CTA) */}
          <div className="w-full lg:w-1/3 xl:w-[32%] flex flex-col gap-4">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col gap-3">
              {(() => {
                ...
                const scheduledEnd = test.settings?.scheduledEndTime || (scheduledStart + (displayTimeLimit || 3600) * 1000);
                const isBefore = isScheduled && currentTime < scheduledStart;
                const hasAttempted = testAttempts.length > 0;
                const isEnded = isScheduled && (currentTime >= scheduledEnd || hasAttempted);
                  ...
                  return (
                    <div className="flex flex-col gap-3">
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-center">
                        <div className="flex items-center justify-center gap-1.5 text-amber-700 font-bold text-xs uppercase tracking-wider mb-1">
                        ...
                        <div className="text-[10px] uppercase font-bold tracking-wider text-amber-700 mt-1">
                      ...
                      <button
                        ...
                        className="w-full h-10 rounded-lg bg-amber-500 text-white flex items-center justify-center gap-2 hover:bg-amber-600 transition-colors shadow-xs font-semibold text-sm cursor-pointer"
                      >
                  ...
                  return (
                    <div className="flex flex-col gap-3">
                      <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                        <div className="flex items-center justify-center gap-1.5 text-slate-700 font-bold text-xs uppercase tracking-wider mb-1">
                          <AlertCircle className="w-3.5 h-3.5 text-slate-500" />
                          <span>{hasAttempted ? 'Test Completed' : 'Scheduled Window Ended'}</span>
                        </div>
                        <p className="text-[11px] text-slate-500 font-medium mt-1">
                          {hasAttempted ? 'You have successfully submitted your test.' : `Closed on ${new Date(scheduledEnd).toLocaleDateString()} at ${new Date(scheduledEnd).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.`}
                        </p>
                      </div>
                      ...
                      {latestAttempt ? (
                        <button
                          ...
                          className="w-full h-10 rounded-lg bg-emerald-600 text-white flex items-center justify-center gap-2 hover:bg-emerald-700 transition-colors shadow-xs font-semibold text-sm cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="w-full h-10 rounded-lg bg-blue-600 text-white flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors shadow-xs font-semibold text-sm cursor-pointer"
                        >
                ...
                if (activeTestSessions && activeTestSessions[test.id]) {
                  ...
                  const sessionMinsLeft = Math.floor((session.timeLeft ?? displayTimeLimit) / 60);
                  const sessionSecsLeft = (session.timeLeft ?? displayTimeLimit) % 60;
                  ...
                  return (
                    <>
                      ...
                      <button
                        ...
                        className="w-full h-9 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors flex items-center justify-center gap-2 font-semibold text-xs border border-slate-200 cursor-pointer"
                      >
                        <RotateCcw className="w-3 h-3 text-slate-500" />
                        <span>Start Fresh ({Math.floor(displayTimeLimit / 60)} Mins)</span>
                      </button>
                    </>
                  );
                }
                ...
                return (
                  <button
                    onClick={() => navigate(`/test/${test.id}`)}
                    className="w-full h-10 rounded-lg bg-blue-600 text-white flex items-center justify-center gap-2 hover:bg-blue-700 transition-colors shadow-xs cursor-pointer"
                  >
                    <PlayCircle className="w-4 h-4" />
              ...
              <button
                onClick={() => navigate(`/test-answers/${test.id}`)}
                className="text-xs font-bold text-slate-600 hover:text-blue-600 transition-colors flex items-center justify-center gap-1 w-full h-8 cursor-pointer"
              >
              ...
              {isOwner && (
                <button
                  ...
                  className="text-xs font-bold text-indigo-700 hover:text-indigo-900 transition-colors flex items-center justify-center gap-1.5 w-full h-9 bg-indigo-50 hover:bg-indigo-100 rounded-lg border border-indigo-100 cursor-pointer"
                >
                  <Edit3 className="w-3.5 h-3.5" /> Edit Test Content
                </button>
              )}
            </div>

            {/* Test Information / Scheme */}
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
              ...
              <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
                <span className="text-slate-500 font-medium">Marking Scheme</span>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
              <div className="flex items-center justify-between mb-3">
                ...
                {latestAttempt && (
                  <button
                    ...
                    className="p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors flex items-center gap-1 text-[11px] font-bold cursor-pointer"
              ...
              {latestAttempt ? (
                <div className="flex flex-col">
                  ...
                  <button 
                    onClick={() => navigate(`/review/${latestAttempt.id}`)}
                    className="w-full h-8 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100 transition-colors cursor-pointer"
                  >
                    View Analysis
                  </button>
