...
const oldStr = `      if (Array.isArray(parsed) && parsed.length > 0 && parsed[0]?.title && Array.isArray(parsed[0]?.questions)) {
        newTests = parsed.map((test: any) => ({
          ...test,
          id: test.id || uuidv4(),
          positiveMarks: test.positiveMarks !== undefined ? Number(test.positiveMarks) : (test.examCategory === 'SSC CGL' ? 2.0 : 1.0),
          negativeMarks: test.negativeMarks !== undefined ? Number(test.negativeMarks) : (test.examCategory === 'SSC CGL' ? 0.5 : 0.25),
...
const newStr = `      if (Array.isArray(parsed) && parsed.length > 0 && parsed[0]?.title && Array.isArray(parsed[0]?.questions)) {
        newTests = parsed.map((test: any) => {
...
if (code.includes(oldStr)) {
  console.log("Found old str, replacing");
  code = code.replace(oldStr, newStr);
  fs.writeFileSync('src/pages/QuestionBank.tsx', code);
} else {
  console.log("Could not find old str");
}
